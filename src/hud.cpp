...
void hud_draw_2d_hud() {
    ...
    for (SZ i = 0; i < WORLD_MAX_ENTITIES; ++i) {
        if (g_world->type[i] == ENTITY_TYPE_BUILDING_LUMBERYARD) {
            total_wood += g_world->building[i].lumberyard.wood_count;
        }
    }

    F32 const alpha   = 1.0F;
    F32 const padding = ui_scale_x(0.75F);
    F32 const spacing = -ui_scale_x(1.25F);  // Padding between icon and text
    ...
    // Position icon in top-right with padding
    Vector2 const icon_pos = {
        ((F32)c_video__render_resolution_width) - icon_size - padding,
        padding
    };

    d2d_texture_ex_raylib(
                          icon,
                          icon_pos,
                          tilt_angle,
                          icon_size / (F32)icon.width,
                          Fade(SUNSETAMBER, alpha)
                         );

    // Count text to the left of icon
    AFont *count_font        = asset_get_font("GoMono", ui_font_size(2.5F));
    String const *count_text = TS("%zu", total_wood);
    Vector2 const count_size = measure_text(count_font, count_text->c);

    Vector2 const count_pos = {
        icon_pos.x - count_size.x - spacing,
        icon_pos.y + ((icon_size - count_size.y) / 2.0F)
    };
