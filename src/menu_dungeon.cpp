...
C8 const *menu_extra_dungeon_func_get_label(void *cb_data, SZ idx, Color *color_ptr, BOOL *color_override_ptr) {  // NOLINT
    unused(cb_data);
    unused(color_ptr);
...
C8 const *menu_extra_dungeon_func_get_desc(void *cb_data, SZ idx, Color *color_ptr, BOOL *color_override_ptr) {  // NOLINT
    unused(cb_data);
    unused(color_ptr);
    unused(color_override_ptr);
...
void menu_extra_dungeon_func_change(void *_cb_data, BOOL increase) {
    ...
    switch (selected_entry) {
        case MENU_CHOICE_ADD_REMOVE_RANDOM_NPC: {
            if (increase) {
                entity_spawn_npc(count, report);
            } else {
                entity_despawn_npc(count, report);
            }
        } break;
        case MENU_CHOICE_ADD_REMOVE_VEGETATION: {
            if (increase) {
        ...
        default: {
            ...
            menu_extra_dungeon_func_yes(_cb_data);
        } break;
    }
}
